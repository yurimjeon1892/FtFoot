...
import data_loader
from common.utils import resize_rgb_image, save_image
...
def main():
    ...
    test_dataset = data_loader.__dict__[config.data_config.name](
        cfg=config.data_config, mode="test"
    )
    ...
    out_dir = os.path.join(
        "./outputs/prediction", rn + "-" + cn + "-" + config.data_config.name
    )
...
def save_trav_result(save_path, dataset, fname, preds):
    ...
    if dataset == "RELLIS_3D":
        for b in range(len(fname)):
            ...
            print(np.min(trav_b), np.max(trav_b), trav_b.shape)
            # trav_b = 1 - trav_b
