...
from common.utils import AverageMeter, feat_random_aug, rgbd_random_aug
...
def main():
    ...
    train_loader, val_loader = init_dataset(
        config.data_config, config.batch_size, config.num_workers
    )
    ...
    for epoch in range(start_epoch, config.end_epoch + 1):
        ...
        is_best = False
        if config.data_config.name == "ORFD":
            val_metric = valid_orfd(
                model, val_loader, criterions, summary, epoch, config.loss_config
            )
...
def train(model, loader, criterions, optimizer, summary, epoch, loss_config):
    ...
    for batch_idx, (rgbd, gts, fname) in enumerate(
        tqdm(loader, desc=description, unit="batches")
    ):
        ...
        pred_up = F.interpolate(
            preds["trav"], size=input_size, mode="bilinear", align_corners=True
        )
        ...
        loss = (
            loss_ss * loss_config.lamb_ss
            + loss_ce * loss_config.lamb_ce
            + loss_sn * loss_config.lamb_sn
        )
    ...
    update_summary(
        summary, rgbd, gts, preds, metric_dict, rgbd_ss, preds_ss, epoch, "train"
    )
...
def valid(model, loader, criterions, summary, epoch, loss_config):
    ...
    with torch.no_grad():
        for _, (rgbd, gts, fname) in enumerate(
            tqdm(loader, desc=description, unit="batches")
        ):
            input_size = rgbd.size()[2:4]
            rgbd = rgbd.cuda().float()
    ...
    update_summary(
        summary, rgbd, gts, preds, metric_dict, rgbd_ss, preds_ss, epoch, "valid"
    )
    for k in metric_dict.keys():
        print(k + ": {:.4f},".format(metric_dict[k]), end=" ")
    print()
...
def valid_orfd(model, loader, criterions, summary, epoch, loss_config):
    ...
    with torch.no_grad():
        for _, (rgbd, gts, fname) in enumerate(
            ...
            pred_trav_np = F.interpolate(
                pred_trav_np, size=raw_cam_img_size, mode="bilinear", align_corners=True
            )
