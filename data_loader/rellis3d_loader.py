...
class RELLIS_3D(data.Dataset):
    def __init__(self, cfg, mode="test"):
        ...
        if len(self.samples) == 0:
            raise (
                RuntimeError("Found 0 files in subfolders of: " + self.data_path + "\n")
            )
    ...
    def make_sample_dataset(self):
        ...
        for seq in self.seqs:
            calib_dict[seq] = {}
            Tr_fn = os.path.join(
                self.data_path, "Rellis_3D", str(seq).zfill(5), "transforms.yaml"
            )
            ...
            P_fn = os.path.join(
                self.data_path, "Rellis-3D", str(seq).zfill(5), "camera_info.txt"
            )
            P = get_cam_mtx(P_fn)
            P_eye = np.eye(4)
            P_eye[:3, :3] = P
            ...
            file_list = os.listdir(
                os.path.join(
                    self.data_path, "Rellis-3D", str(seq).zfill(5), "pylon_camera_node"
                )
            )
        ...
        for seq in self.seqs:
            ...
            file_list = os.listdir(
                os.path.join(
                    self.data_path, "Rellis-3D", seq_str, "os1_cloud_node_kitti_bin"
                )
            )
            file_list = natsort.natsorted(file_list)

            file_list = file_list[
                self.frame_crop_idx[seq][0] : self.frame_crop_idx[seq][1]
            ]  # first / last frames don't have footprints..
    ...
    def __getitem__(self, index):
        ...
        im_np = resize_rgb_image(
            im_np,
            (
                int(self.raw_cam_img_size[0] / self.ratio),
                int(self.raw_cam_img_size[1] / self.ratio),
            ),
        )
        ...
        # preprocess calib
        calib_mtx = (
            self.samples[index]["calib"]["P"] @ self.samples[index]["calib"]["Tr"]
        )

        # preprocess depth
        depth_np = depth_img_from_cartesian_pc_numpy(
            pc_np, calib_mtx, self.raw_cam_img_size
        )
