...
from common.utils import pcd_read, rgb_read, save_image
...
def main():
    ...
    parser.add_argument(
        "--data_root",
        type=str,
        help="Path to the rellis dataset",
        default="/storage2/public/rellis-3d",
    )
    ...
    parser.add_argument("--pcd_range", type=float, help="pcd range", default=30.0)
    ...
    parser.add_argument(
        "--cost_path", type=str, help="Path to the cost map", default=""
    )
    ...
    for sample_one in tqdm(sample_list):
        ...
        if args.cost_path != "":
            file_path_tmp = os.path.join(
                args.cost_path,
                seq_str + "_" + str(sample_one["fnum"]).zfill(6) + ".png",
            )
        ...
        pc_in_fov = np.concatenate(
            [pc_in_fov.T[:3, :], np.ones((1, pc_in_fov.shape[0]))], 0
        )
    ...
    max_x, min_x = (
        np.max(pose_xyz_all[:, 0]) + args.pcd_range + 2.5,
        np.min(pose_xyz_all[:, 0]) - args.pcd_range / 2 - 2.5,
    )
    max_y, min_y = (
        np.max(pose_xyz_all[:, 1]) + args.pcd_range / 2 + 2.5,
        np.min(pose_xyz_all[:, 1]) - args.pcd_range - 2.5,
    )
    H, W = (
        int((max_x - min_x) * args.meter_to_grid),
        int((max_y - min_y) * args.meter_to_grid),
    )
    print(min_x, min_y)
    # exit()
    ...
    if args.save_traj_img:
        footprint_map = draw_map(
            pose_xyz_all,
            np.zeros_like(pose_xyz_all),
            H,
            W,
            min_x,
            min_y,
        ...
        for i in range(pose_xyz_all.shape[0]):
            px, py = (
                int((pose_xyz_all[i, 0] - min_x) * args.meter_to_grid),
                int((pose_xyz_all[i, 1] - min_y) * args.meter_to_grid),
            )
    ...
    if args.save_rgb_img:
        rgb_map = draw_map(
            xyz_all, rgb_all, H, W, min_x, min_y, args.meter_to_grid, args.pixel_size
        )
    ...
    if args.save_valid_map:
        valid_ground = draw_map(
            xyz_all,
            np.ones_like(xyz_all),
            H,
            W,
            min_x,
            min_y,
...
def draw_map(xyz_all, rgb_all, H, W, min_x, min_y, meter_to_grid, pixel_size, canvas=1):
    ...
    for i in range(xyz_all.shape[0]):
        px, py = (
            int((xyz_all[i, 0] - min_x) * meter_to_grid),
            int((xyz_all[i, 1] - min_y) * meter_to_grid),
        )
