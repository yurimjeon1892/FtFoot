...
from .utils_loader import get_cam_mtx, get_lidar2cam_mtx, pose_read
...
def read_raw_sample_list(args):
    ...
    file_list = os.listdir(
        os.path.join(args.data_root, "Rellis-3D", seq_str, "os1_cloud_node_kitti_bin")
    )
    ...
    for fn in file_list:
        ...
        indiv_sample = {
            "image": os.path.join(
                ...
                seq_str,
                "pylon_camera_node",
                cam_name_dict_seq[str_frame] + ".jpg",
            ),
            "point": os.path.join(
                args.data_root,
                "Rellis-3D",
