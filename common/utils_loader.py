...
def crop_image(img, target_size, init=False):
    ...
    h = img.shape[0]
    w = img.shape[1]
    i = int(math.floor((h - target_size[0]) / 2.0))
    j = int(math.floor((w - target_size[1]) / 2.0))
    if init:
        i, j = 0, 0
...
def resize_depth_image(img, target_size):
    ...
    for h in range(img.shape[0]):
        for w in range(img.shape[1]):
            if img[h, w] != 0:
                out_img[int(h * h_ratio), int(w * w_ratio)] = img[h, w]
    return out_img
    ...
    for h in range(img.shape[0]):
        for w in range(img.shape[1]):
            for c in range(img.shape[2]):
                if img[h, w, c] != -1:
                    out_img[
                        int(h * h_ratio) : int((h + 1) * h_ratio),
                        int(w * w_ratio) : int((w + 1) * w_ratio),
                        c,
                    ] = img[h, w, c]
    return out_img
...
def sn_image_from_npy(sn_np, raw_cam_img_size, px):
    ...
    for i in range(sn_np.shape[0]):
        y_min, y_max = (
            np.maximum(0, int(sn_np[i, 0]) - px),
            np.minimum(raw_cam_img_size[0] - 1, int(sn_np[i, 0]) + px + 1),
        )
        x_min, x_max = (
            np.maximum(0, int(sn_np[i, 1]) - px),
            np.minimum(raw_cam_img_size[1] - 1, int(sn_np[i, 1]) + px + 1),
        )
