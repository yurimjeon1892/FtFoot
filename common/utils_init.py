...
def save_state(model, epoch, best_metric, is_best, out_dir, filename="checkpoint.pth"):
    ...
    state_dict = {"model": model_state_dict, "epoch": epoch, "best_metric": best_metric}
    ...
    if is_best:
        shutil.copyfile(
            os.path.join(out_dir, filename), os.path.join(out_dir, "model_best.pth")
        )
...
def init_model(cfg_model):
    ...
    print(
        "[i] complete init_model: {:.3f} M".format(
...
def init_dataset(cfg_data, batch_size, num_workers):
    train_dataset = data_loader.__dict__[cfg_data.name](cfg=cfg_data, mode="train")
    ...
    val_dataset = data_loader.__dict__[cfg_data.name](cfg=cfg_data, mode="valid")
