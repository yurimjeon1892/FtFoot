...
class Baseline(nn.Module):
    def __init__(
        ...
        self.layer2_0 = nn.Conv2d(
            in_channels * 2, 3, kernel_size=3, stride=1, padding=1
        )
        ...
        self.inplanes = in_channels * 4 * block.expansion
        self.layer3_3 = self._make_layer(
            block, in_channels * 8, depth_layers[2], stride=2
        )
        self.inplanes = in_channels * 8 * block.expansion
        self.layer3_4 = self._make_layer(
            block, in_channels * 8, depth_layers[3], stride=2
        )
        self.inplanes = in_channels * 8 * block.expansion
        self.layer3_5 = self._make_layer(
        ...
        self.layer5_1 = block(
            in_channels * 8, in_channels * 8, norm_layer=norm_layer, act=False
        )
        self.layer5_2 = nn.Sequential(
            nn.Dropout2d(0.1, False), nn.Conv2d(in_channels * 8, 2, 1)
        )
...
def STANDARD4():
    return Baseline(
        norm_layer=encoding.nn.SyncBatchNorm, guide=STANDARD, weight_ks=3, bc=4
    )


def GUIDE4():
    return Baseline(
        norm_layer=encoding.nn.SyncBatchNorm, guide=GUIDE, weight_ks=1, bc=4
    )
