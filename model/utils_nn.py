...
class RW_Module(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.shrink_factor != 1:
            ...
            x_shrink = Func.interpolate(
                x_shrink, size=(height, width), mode="bilinear", align_corners=True
            )

        proj_query = (
            self.query_conv(x_shrink)
...
class GFL(nn.Module):
    def __init__(
        self, input_planes, weight_planes, norm_layer=None, weight_ks=3, return_ks=False
    ):
        super().__init__()
