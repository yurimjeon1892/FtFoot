...
if __name__ == "__main__":
    ...
    for seq in args.seqs:
        ...
        save_root = os.path.join(
            args.data_root, "Rellis-3D-custom", str(seq).zfill(5), "super_pixel"
        )
        os.makedirs(save_root, exist_ok=True)

        file_list = os.listdir(
            os.path.join(args.data_root, "Rellis-3D", seq_str, "pylon_camera_node")
        )
        for fn in tqdm(file_list):
            rgb_path = os.path.join(
                args.data_root, "Rellis-3D", seq_str, "pylon_camera_node", fn
            )
