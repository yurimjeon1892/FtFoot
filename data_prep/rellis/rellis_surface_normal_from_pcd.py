...
from common.utils_loader import (get_cam_mtx, get_lidar2cam_mtx, pcd_read,
                                 sn_image_from_npy)

if __name__ == "__main__":
    # Create object for parsing command-line options
    parser = argparse.ArgumentParser(
        description="Generate surface normal from point cloud"
    )
    ...
    for seq in args.seqs:
        ...
        f = open(
            os.path.join(args.data_root, "Rellis-3D", str(seq).zfill(5), "poses.txt"),
            "r",
        )
        poses = f.readlines()
        f.close()
        ...
        save_root = os.path.join(
            args.data_root, "Rellis-3D-custom", str(seq).zfill(5), "surface_normal"
        )
        os.makedirs(save_root, exist_ok=True)

        save_root_clr = os.path.join(
            args.data_root, "Rellis-3D-custom", str(seq).zfill(5), "surface_normal_2"
        )
