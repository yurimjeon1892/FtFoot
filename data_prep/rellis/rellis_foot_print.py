...
from common.utils_loader import (get_cam_mtx, get_lidar2cam_mtx, pose_read,
                                 rgb_read)
...
def make_sample_dataset(args):
    ...
    for seq in args.seqs:
        calib_dict[seq] = {}
        Tr_fn = os.path.join(
            args.data_root, "Rellis_3D", str(seq).zfill(5), "transforms.yaml"
        )
        ...
        P_fn = os.path.join(
            args.data_root, "Rellis-3D", str(seq).zfill(5), "camera_info.txt"
        )
        P = get_cam_mtx(P_fn)
        P_eye = np.eye(4)
        P_eye[:3, :3] = P
        ...
        file_list = os.listdir(
            os.path.join(
                args.data_root, "Rellis-3D", str(seq).zfill(5), "pylon_camera_node"
            )
        )
    ...
    for seq in args.seqs:
        ...
        file_list = os.listdir(
            os.path.join(
                args.data_root, "Rellis-3D", seq_str, "os1_cloud_node_kitti_bin"
            )
        )
...
def plot_foot_print(foot_prints_pc, calib_mtx, raw_cam_img_size, binary=True):
    ...
    foot_prints_one = np.concatenate(
        [foot_prints_pc, np.ones((foot_prints_pc.shape[0], 1))], -1
    )
    ...
    for _, xyw in enumerate(cj_on_Pi.T):
        ...
        if is_in_img:
            ...
            if binary:
                foot_print_map[yy, xx] = 1
            else:
                foot_print_map[yy, xx] = w

    if binary:
        foot_print_map = np.tile(np.expand_dims(foot_print_map, -1), (1, 1, 3))
...
if __name__ == "__main__":
    ...
    for seq in args.seqs:
        ...
        save_root = os.path.join(
            args.data_root, "Rellis-3D-custom", seq_str, "foot_print"
        )
        os.makedirs(save_root, exist_ok=True)
        save_root_2 = os.path.join(
            args.data_root, "Rellis-3D-custom", seq_str, "foot_print_2"
        )
    ...
    for sample_one in tqdm(sample_list):
        ...
        foot_prints_pc = make_foot_prints_pc(
            sample_one["Pijs"], num_cline=10, rad_circle=0.5
        )

        fp_np = plot_foot_print(
            foot_prints_pc, calib_mtx, raw_cam_img_size, binary=True
        )
        save_image(
            fp_np * 255,
            os.path.join(
                sample_one["save_path"], "foot_print", sample_one["fname"] + ".png"
            ),
        )
