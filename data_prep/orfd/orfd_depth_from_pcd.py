...
from common.utils_loader import bin_read, pose_read_orfd, read_calib_file_orfd


def search_for_accumulation(
    data_path, mode, seq_list, poses, seq_i, seq_sample_num, P_oi, stride
):
    ...
    while len(pc_np_list) < 10:
        ...
        pc_j = np.concatenate(
            (pc_j[:3, :], np.ones((1, pc_j.shape[1]), dtype=pc_j.dtype)), axis=0
        )
        pc_j = P_ij @ pc_j
        pc_j = pc_j[:3, :]
...
def get_accumulated_pc(data_path, mode, seq_list, poses, seq_i):
    ...
    # search for previous
    prev_pc_np_list = search_for_accumulation(
        data_path, mode, seq_list, poses, seq_i, seq_sample_num, P_oi, -1
    )
    # search for next
    next_pc_np_list = search_for_accumulation(
        data_path, mode, seq_list, poses, seq_i, seq_sample_num, P_oi, 1
    )
...
if __name__ == "__main__":
    ...
    for seq_name in seq_dict.keys():
        ...
        for i, str_frame in enumerate(seq_dict[seq_name]):
            ...
            calib_mtx = read_calib_file_orfd(
                os.path.join(
                    args.data_root,
                    "Final_Dataset",
                    args.mode,
            ...
            d_img, mask = minmax_color_img_from_img_numpy(
                d_np, plt.cm.plasma, px=2, valid_mask=True
            )
