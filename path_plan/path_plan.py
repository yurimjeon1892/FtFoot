...
def main():
    ...
    parser.add_argument("--t_weight", type=float, default=20.0)
...
def read_start_goal_pos(args, offset_num=50):
    ...
    x0, y0 = (
        int((pose_xyz_all[offset_num, 0] - min_x) * args.meter_to_pixel),
        int((pose_xyz_all[offset_num, 1] - min_y) * args.meter_to_pixel),
    )
    x1, y1 = (
        int((pose_xyz_all[-1, 0] - min_x) * args.meter_to_pixel),
        int((pose_xyz_all[-1, 1] - min_y) * args.meter_to_pixel),
    )
...
def make_path(args, start, goal, cost_map, bound):
    ...
    lp_planner = get_lp_planner(
        type=args.local_planner_type,
