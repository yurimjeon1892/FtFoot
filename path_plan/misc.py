...
def get_local_map(state, global_height_map, detection_range, args):
    ...
    max_x, max_y = (
        min(cx + margin, args["map_width"]),
        min(cy + margin, args["map_height"]),
    )
...
def set_local_camera(p, robot, distance=4, yaw_offset=0, pitch_offset=-35):
    ...
    p.resetDebugVisualizerCamera(
        distance, cameraYaw=yaw, cameraPitch=pitch_offset, cameraTargetPosition=pos
    )
...
def load_env_yaml(envdir, args):
    ...
    post_fix = (
        ...
        + str(args["scale"])
        + "_r"
        + str(angle["rolling_degree"])
        + "_p"
        + str(angle["pitching_degree"])
        + "_y"
        + str(angle["yawing_degree"])
        + "_f"
    ...
    with open(
        os.path.join(
            ...
            "humji",
            args["terrain_name"],
            args["terrain_name"] + post_fix + ".yaml",
        )
    ) as f:
...
class State:
    def __init__(
        self, x=0.0, y=0.0, z=0.0, yaw=0.0, v=0.0, quat=None, numpy_state=None
    ):
    ...
    def __sub__(self, state):
        ...
        yaw = self.yaw - state.yaw
        v = self.v - state.v
        quat = MultiplyQuaternion(
            [state.quat[0], -state.quat[1], -state.quat[2], -state.quat[3]], self.quat
        ).tolist()
        return State(x, y, z, yaw, v, quat)
